...
contract SwouponScript is Script, DeployPermit2 {
    ...
    function run() public {
        ...
        (address hookAddress, bytes32 salt) =
            HookMiner.find(CREATE2_DEPLOYER, permissions, type(Swoupon).creationCode, abi.encode(address(manager)));

        // ----------------------------- //
        // Deploy the hook using CREATE2 //
        // ----------------------------- //
        vm.broadcast();
        Swoupon swoupon = new Swoupon{salt: salt}(manager, "Swoupon", "SWP");
        require(address(swoupon) == hookAddress, "SwouponScript: hook address mismatch");
